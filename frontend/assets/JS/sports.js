// Sports Category Page JavaScript
...
async function fetchProducts() {
    try {
        console.log('Sports: Starting to fetch products...');
        
        const grid = document.getElementById('sportsGrid');
        if (!grid) {
            console.error('Sports: sportsGrid element not found!');
            return;
        }
        
        // Check cache first
        const cachedData = sessionStorage.getItem(`sports_products_${SPORTS_CATEGORY_ID}`);
        if (cachedData) {
            console.log('Sports: Using cached products');
            const cached = JSON.parse(cachedData);
            sportsProducts = cached;
            filteredProducts = [...sportsProducts];
            displayProducts();
            return;
        }
        
        console.log('Sports: Grid element found');
        grid.innerHTML = '<div class="col-span-full text-center py-12"><div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600"></div><p class="text-gray-600 mt-4">Loading products...</p></div>';
        
        const url = `${BACKEND_URL}/category/${SPORTS_CATEGORY_ID}`;
        console.log('Sports: Fetching from URL:', url);
        ...
        console.log('Sports: Response status:', response.status, 'OK:', response.ok);
        ...
        const data = await response.json();
        console.log('Sports: Number of products received:', data.products ? data.products.length : 0);
        
        // Transform backend data to match frontend structure
        sportsProducts = data.products.map(product => {
            const subcategory = determineSubcategory(product.category_id, product.name);
            console.log(`Sports: Product "${product.name}" (category_id: ${product.category_id}) -> subcategory: ${subcategory}`);
        ...
        console.log('Sports: Transformed products count:', sportsProducts.length);
        
        // Cache products for 5 minutes
        sessionStorage.setItem(`sports_products_${SPORTS_CATEGORY_ID}`, JSON.stringify(sportsProducts));
        
        filteredProducts = [...sportsProducts];
        console.log('Sports: Calling displayProducts()');
        displayProducts();
    } catch (error) {
        if (error.name === 'AbortError') {
            console.error('Sports: Request timeout');
            showErrorMessage('Request timed out. Please check your connection and try again.');
        } else {
            console.error('Sports: Error fetching products:', error);
...
function determineSubcategory(categoryId, productName) {
    ...
    if (nameLower.includes('dumbbell') || nameLower.includes('treadmill') || nameLower.includes('resistance') || nameLower.includes('fitness')) {
        return 'fitness';
    } else if (nameLower.includes('camping') || nameLower.includes('hiking') || nameLower.includes('outdoor') || nameLower.includes('tent')) {
        return 'outdoor';
    } else if (nameLower.includes('bike') || nameLower.includes('cycling') || nameLower.includes('helmet')) {
        return 'cycling';
    } else if (nameLower.includes('yoga') || nameLower.includes('mat') || nameLower.includes('meditation')) {
        return 'yoga';
    } else {
        return 'team-sports';
    }
...
function displayProducts() {
    console.log('Sports: displayProducts called');
    
    const grid = document.getElementById('sportsGrid');
    const noProductsMsg = document.getElementById('noProducts');
    
    if (!grid) {
        console.error('Sports: sportsGrid element not found in displayProducts!');
        return;
    }
    
    console.log('Sports: Current subcategory:', currentSubcategory);
    console.log('Sports: All products count:', sportsProducts.length);
    
    filteredProducts = currentSubcategory === 'all' 
        ? [...sportsProducts]
        : sportsProducts.filter(p => p.subcategory === currentSubcategory);
    
    console.log('Sports: Filtered products count:', filteredProducts.length);
        ...
        <div class="product-card bg-white rounded-2xl shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-2xl group">
            <div class="relative overflow-hidden">
                ...
                <div class="absolute top-0 left-0 right-0 p-3 flex justify-between items-start">
                    <span class="discount-badge bg-gradient-to-r from-orange-500 to-red-500 text-white px-3 py-1.5 rounded-full text-sm font-bold shadow-lg">
                        ${discount}% OFF
                    </span>
            ...
            <div class="p-5">
                <div class="mb-3">
                    <h3 class="text-xl font-bold text-gray-800 mb-1 line-clamp-1 group-hover:text-orange-600 transition-colors">${product.name}</h3>
                    <span class="inline-block bg-orange-100 text-orange-600 text-xs px-2 py-1 rounded-full font-medium">Sports</span>
                </div>
                ...
                <div class="mb-4 flex items-baseline gap-2">
                    <span class="price-tag text-3xl font-extrabold bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">
                        $${product.price}
                    </span>
                ...
                <div class="flex gap-2">
                    <button onclick="addToCart(${product.id})" class="flex-1 bg-gradient-to-r from-orange-600 to-red-600 text-white px-4 py-3 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 flex items-center justify-center gap-2">
...
function addToCart(productId) {
    console.log('🛒 Sports: addToCart called with product ID:', productId);
