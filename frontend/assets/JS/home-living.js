...
// Category ID for home & living
const HOME_LIVING_CATEGORY_ID = 3;
...
async function fetchProducts() {
    try {
        console.log('Home & Living: Starting to fetch products...');
        
        const grid = document.getElementById('homeLivingGrid');
        if (!grid) {
            console.error('Home & Living: homeLivingGrid element not found on page load!');
            return;
        }
        
        // Check cache first
        const cachedData = sessionStorage.getItem(`homeliving_products_${HOME_LIVING_CATEGORY_ID}`);
        if (cachedData) {
            console.log('Home & Living: Using cached products');
            const cached = JSON.parse(cachedData);
            homeLivingProducts = cached;
            filteredProducts = [...homeLivingProducts];
            displayProducts();
            return;
        }
        
        console.log('Home & Living: Grid element found, showing loading state');
        
        // Show loading state with spinner
        grid.innerHTML = '<div class="col-span-full text-center py-12"><div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-green-600"></div><p class="text-gray-600 mt-4">Loading products...</p></div>';
        
        const url = `${BACKEND_URL}/category/${HOME_LIVING_CATEGORY_ID}`;
        console.log('Home & Living: Fetching from URL:', url);
        ...
        console.log('Home & Living: Response status:', response.status, 'OK:', response.ok);
        ...
        const data = await response.json();
        console.log('Home & Living: Number of products received:', data.products ? data.products.length : 0);
        
        // Transform backend data to match frontend structure
        homeLivingProducts = data.products.map(product => {
            const subcategory = determineSubcategory(product.category_id, product.name);
            console.log(`Home & Living: Product "${product.name}" (category_id: ${product.category_id}) -> subcategory: ${subcategory}`);
        ...
        console.log('Home & Living: Transformed products count:', homeLivingProducts.length);
        
        // Cache products for 5 minutes
        sessionStorage.setItem(`homeliving_products_${HOME_LIVING_CATEGORY_ID}`, JSON.stringify(homeLivingProducts));
        
        filteredProducts = [...homeLivingProducts];
        console.log('Home & Living: Calling displayProducts()');
        displayProducts();
    } catch (error) {
        if (error.name === 'AbortError') {
            console.error('Home & Living: Request timeout');
            showErrorMessage('Request timed out. Please check your connection and try again.');
        } else {
            console.error('Home & Living: Error fetching products:', error);
...
function determineSubcategory(categoryId, productName) {
    ...
    if (nameLower.includes('sofa') || nameLower.includes('chair') || nameLower.includes('table') || nameLower.includes('desk')) {
        return 'furniture';
    } else if (nameLower.includes('coffee') || nameLower.includes('blender') || nameLower.includes('cookware') || nameLower.includes('kitchen')) {
        return 'kitchen';
    } else if (nameLower.includes('lamp') || nameLower.includes('light') || nameLower.includes('chandelier')) {
        return 'lighting';
    } else if (nameLower.includes('pillow') || nameLower.includes('bedding') || nameLower.includes('blanket') || nameLower.includes('sheet')) {
        return 'bedding';
    } else {
        return 'decor';
    }
...
function filterBySubcategory(subcategory) {
    ...
    event.target.closest('button').classList.add('bg-green-600', 'text-white');
    displayProducts();
}
...
function displayProducts() {
    console.log('Home & Living: displayProducts called');
    
    const grid = document.getElementById('homeLivingGrid');
    const noProductsMsg = document.getElementById('noProducts');
    
    if (!grid) {
        console.error('Home & Living: homeLivingGrid element not found!');
        return;
    }
    
    console.log('Home & Living: Current subcategory:', currentSubcategory);
    console.log('Home & Living: All products count:', homeLivingProducts.length);
    
    filteredProducts = currentSubcategory === 'all' 
        ? [...homeLivingProducts]
        : homeLivingProducts.filter(p => p.subcategory === currentSubcategory);
    
    console.log('Home & Living: Filtered products count:', filteredProducts.length);
        ...
        <div class="product-card bg-white rounded-2xl shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-2xl group">
            <div class="relative overflow-hidden">
                ...
                <div class="absolute top-0 left-0 right-0 p-3 flex justify-between items-start">
                    <span class="discount-badge bg-gradient-to-r from-green-500 to-emerald-500 text-white px-3 py-1.5 rounded-full text-sm font-bold shadow-lg">
                        ${discount}% OFF
                    </span>
            ...
            <div class="p-5">
                <div class="mb-3">
                    <h3 class="text-xl font-bold text-gray-800 mb-1 line-clamp-1 group-hover:text-green-600 transition-colors">${product.name}</h3>
                    <span class="inline-block bg-green-100 text-green-600 text-xs px-2 py-1 rounded-full font-medium">Home & Living</span>
                </div>
                ...
                <div class="mb-4 flex items-baseline gap-2">
                    <span class="price-tag text-3xl font-extrabold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                        $${product.price}
                    </span>
                ...
                <div class="flex gap-2">
                    <button onclick="addToCart(${product.id})" class="flex-1 bg-gradient-to-r from-green-600 to-emerald-600 text-white px-4 py-3 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 flex items-center justify-center gap-2">
...
function addToCart(productId) {
    console.log('🛒 Home&Living: addToCart called with product ID:', productId);
    
    const product = homeLivingProducts.find(p => p.id === productId);
    if (!product) {
